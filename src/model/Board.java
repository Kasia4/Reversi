...
public class Board {
	 ...
	 * @return
	 */
	public float heuristicTest(){
	    float sum = 0;
	    Player player = new Player(Pawn.WHITE);//tylko do testów
	    int b = 0;
	    float v = 5;
	    for(int x = 0; x < BoardSize.SMALL.size; x++)
	        for(int y = 0; y < BoardSize.SMALL.size; y++){
	            ...
	            sum += b * getValueOnBoard(new Vector2(x,y));
	        }
	    sum += v * availableFieldsNumber(player.getPawn());
	    return sum;
	}
	/**
	 ...
	 */
	private float getValueOnBoard(Vector2 vec){
	    float a = 53.15f;
	    float b = -32.97f;
	    float c = -43.33f;
	    float d = 24.61f;
